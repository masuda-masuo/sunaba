...
| `verify_in_container` | `container_id`, `path`, `test_filter` (opt), `pytest_args` (opt), `language` (opt), `working_dir` (opt) | **Pre-publish gate.** Runs the lint and type gates as a precondition, then the project's tests (`pytest`/`jest`/`go test`/`npm test`). Returns structured test results plus a diff summary. Test counts are reported whenever the runner emits them, including TAP from `node --test`; a run whose output cannot be parsed into counts says so instead of returning a bare success, because an implicit-discovery runner exits 0 even when it found no tests at all. |
