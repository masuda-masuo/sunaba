...
| `edit_symbol` | `container_id`, `file_path`, `symbol`, `new_code`, `line` (opt), `preserve` (opt) | **Symbol edit path.** Locates a function/class/method by name via AST (decorators included) and replaces the whole definition with `new_code`; `new_code=""` deletes it. `preserve` (default `"decorators+docstring"`) keeps old decorators/docstring; `"none"` does full replacement. Comments are never preserved (AST discards them). Nothing is written unless the edited file parses. Python files only. |
