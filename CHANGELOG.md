...
- **Foreground exec deadline with process-tree reaping** (refs #915): `sandbox_exec`, `run_python`, and `package_install` now run under the #910 marker/deadline/reap machinery. Previously a call whose client gave up (MCP transport timeout, or an outer kusabi task timeout) left its Docker exec -- and any ordinary or setsid-detached descendants -- running, because the server cannot cancel the worker thread; the leftovers accumulated against the container's `pids.max`. The deadline is `SUNABA_FOREGROUND_TIMEOUT` (default 270s, `0` disables, invalid/negative fall back to 270); on expiry the marked tree is reaped and the call returns a terminal `status:"timeout"`, `exit_code:124` result with `timeout: {deadline_s, reap, elapsed_s}`, journalled as a single nonzero completion. An explicit `sandbox_exec(timeout=N)` keeps its existing result and now also reaps descendants. `sandbox_exec_background` is intentionally not armed. Behavior change: `package_install` has no per-call override, so an install that previously ran past the client's wait and still completed in the container is now killed at the deadline — run heavy installs via `sandbox_exec_background`. Known limit (shared with the verify deadline): the tree is identified by an environment marker, so a descendant that clears or replaces its environment is invisible to the reap and can outlive `reap:"ok"`.
- **CRLF-terminated output lines are no longer erased** (refs #915): output sanitization treated the `\r` of a `\r\n` line ending as an in-place progress-bar overwrite and kept the (empty) segment after it, so any CRLF line vanished. The visible casualty was Docker's `OCI runtime exec failed: ... procReady not received\r\n` when a saturated container refuses an exec: `sandbox_exec` returned exit 128 with `output: "[×2] "` instead of the reason. Trailing `\r` is now stripped before collapsing, and `compress_repeated_lines` no longer puts a `[×N]` counter on runs of blank lines, which read as one line of content.
- **Sandbox containers run with an init as PID 1** (refs #915, #910): containers were started with `sleep infinity` as PID 1, which never reaps orphans. Every setsid-detached child, and every descendant whose parent was killed (including by the #910 verify deadline's tree reap), was reparented to PID 1 and stayed a zombie for the life of the container -- still counted against `pids_limit=500` -- until unrelated `docker exec` calls failed with OCI `procReady not received`. `build_secure_run_kwargs` now sets `init=True` (Docker's `docker-init`), so orphans are reaped. Measured on the pinned image: five detached orphans left five zombies without init and none with it. Existing containers keep the old PID 1 until recreated.
