...
| `transform_file` | `container_id`, `file_path`, `code`, `max_lines` (opt), `offset` (opt), `limit` (opt) | **Imperative edit path.** Computes new file content via a Python `transform(text) -> str` script inside the container and returns a unified diff. |
