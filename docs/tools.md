...
| Tool Name | Parameters | Description |
|---|---|---|
| `search_in_container` | `container_id`, `pattern`, `path` (opt), `mode` (opt: lexical/structural), `max_results` (opt), `glob` (opt), `ignore_case` (opt), `context` (opt), `output_mode` (opt), `offset` (opt) | Searches code using `ripgrep` (lexical) or AST-based `ast-grep` (structural). Returns structured occurrences. |
