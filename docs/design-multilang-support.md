...
> **注**: この設計書の一部前提は #257 で変更された。scan (semgrep) レイヤーは削除、
> submit は publish にリネームされ内部 verify gate も除去。
> 現在は verify_in_container が test 専用、lint/type は独立ツール。
>
...
- **無音失敗（気づかないエラー）**: ここが最も危険。
  ...
  - `_run_pytest_verify` は出力空でも parse 例外でも `status: skipped` を返す。
    ...
    **テストが一度も走っていないのに publish フローが通る。**
...
- **publish フロー = strict**: 検出した言語に必要な層のどれかが `not_available` か `error` なら
...
| タグ | 構成 | 用途 |
|------|------|------|
| `sandbox:base` | base | 共有親。**検出不能時の中立 fallback**（node + VCS + 検索系。言語ツールチェーンは持たない） |
| `sandbox:python` | base + python + js | **python 検出時**に選択。py 単体 / py+js モノレポ |
| `sandbox:go` | base + go (+ js) | **go 検出時**に選択。go 単体 / go+js |
| `sandbox:full` | base + python + go + js | 混在モノレポ用。opt-in（未ビルド。py+go 検出時も既定にはしない） |
| `sandbox:minimal` | 既存 git+python | 高速起動枠（温存） |

> **既定イメージは「python 固定」ではなく検出ベース（#313）**。当初この表は `sandbox:python` を
> 「`sandbox_initialize` の既定」としていたが、それは本リポジトリが偶々 python だから都合がよいだけで、
> 汎用サンドボックスが普遍的に仮定してよい性質ではない。実装（`code_sandbox_mcp/image_selection.py`）は
> `image` 未指定時にプロジェクトの言語を検出して上表のイメージを選び、**不明・非対応・py+go ポリグロットは
> 中立 `sandbox:base` に fallback**（init はブロックせず notice を返す）。明示 `image=` が最優先。
...
go ツールが 127 → `not_available` → verify `incomplete: true` → publish フローは strict なので fail、
理由「detected go / このイメージ（`sandbox:python@sha…`）に go ツールチェーン無し → `sandbox:go` で再初期化」。
緑にはならない。

`sandbox_initialize` 時の**先回り検出は実装済み**（#313、`image_selection.py`）。コンテナの
イメージは起動後に変えられず、`clone_repo` ではファイルが起動後にしか現れないため、検出は
**起動前にホスト側で**clone 元（Shiori preclone のローカル走査 / なければ GitHub API の root 一覧）を
見て行う。これにより go プロジェクトには最初から `sandbox:go` が選ばれ、verify を待たずにミスマッチを回避する。
それでも誤指定（明示 `image=` で go プロジェクトに `sandbox:python`）が残る場合は、上記 loud-failure 契約が後段で受け止める。
...
- `docker/Dockerfile.sandbox`（Python 固定の全部入り）→ `Dockerfile.base` ＋ `Dockerfile.python` に分割。
  ...
  **#313 で旧 `Dockerfile.sandbox` と `build-sandbox-image.yml`（`_DEFAULT_IMAGE` をモノリスへ巻き戻していた）を削除**。
...
- CI: base を先にビルド → digest 確定 → 子イメージは固定 base に対してビルド → 全部 `@sha256` で publish。
  `build-sandbox-variants.yml` がビルド後に `container.py` の `_NEUTRAL_IMAGE` / `_PYTHON_IMAGE` / `_GO_IMAGE`
  の digest pin を書き戻す PR を自動作成する（#313）。server の既定参照も `:latest` ではなく digest で。
