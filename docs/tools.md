...
| Tool Name | Parameters | Description |
|---|---|---|
| `write_file_sandbox` | `container_id`, `file_name`, `file_contents`, `dest_dir` (opt), `old_str` (opt), `start_line` / `end_line` (opt), `append` (opt), `preserve` (opt), `line` (opt) | **Primary edit path.** With no edit mode given the file is fully overwritten; `old_str` replaces an exact string (on `.py` files, definition-like `old_str` such as ``def foo`` / ``class Bar`` is resolved via AST first for robustness against indentation shifts; an AST no-op returns "No changes", and when `old_str` is a bare signature and `file_contents` a complete definition an AST failure is surfaced instead of falling back to string matching, which would orphan the old body); `start_line`/`end_line` replace a line range, `append=True` appends. The edit modes are mutually exclusive. `preserve` and `line` are AST-only parameters for controlling decorator/docstring preservation and overload disambiguation. |
