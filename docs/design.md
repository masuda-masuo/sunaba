...
*   **Sessions as Organizational Primitives**: Tools are designed not as generic "Docker helpers", but as operations acting upon a **live session (container + filesystem + execution history)**. A feature belongs in this MCP only if it directly manages the state of an active session.
