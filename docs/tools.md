...
| `read_file_range` | `container_id`, `file_path`, `offset` (opt), `limit` (opt), `start_line` (opt), `end_line` (opt) | Reads a slice of a file (pagination by line numbers) to prevent context flooding. |
