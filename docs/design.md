...
> **See [`design_branch_lifecycle.md`](design_branch_lifecycle.md) for how the base commit is chosen.** Clone, diff, merge and publish each have to answer "which commit is where my work started", they resolved it independently, and they disagreed — `diff_in_container` answered `HEAD~1`, which on a fresh clone is whatever landed upstream most recently rather than the caller's own work (#748). The document covers what refs a clone actually provides, the five-layer default-branch ladder (#758), why the review base is the merge target compared from the divergence point rather than from its tip, why untracked files need separate reporting, and the two container-reuse traps that cost real damage (#727). It is deliberately upstream of the merge-handling design above: that one owns what happens once the base is known, this one owns how the base is picked.
