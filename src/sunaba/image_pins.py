...
#: ``FROM`` parent of the variants; ``python`` / ``go`` / ``rust`` / ``js``
#: are the lean images an explicit ``image=`` can still ask for (``js`` added
#: in #588, ``rust`` in #751).
PIN_KEYS: tuple[str, ...] = ("neutral", "python", "go", "rust", "full", "js")
